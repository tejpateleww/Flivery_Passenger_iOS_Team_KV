...
class WalletTransferToBankVC: BaseViewController, SelectBankCardDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.setNavBarWithMenuORBack(Title: "TRANSFER TO BANK".localized, LetfBtn: kIconBack, IsNeedRightButton: false, isTranslucent: false)
        ...
        webserviceForTransferToBank(param as AnyObject) { (result, status) in
            ...
            if (status) {
                ...
                if let res = result as? String {
                    ...
                    UtilityClass.setCustomAlert(title: "Done", message: resDict.object(forKey: GetResponseMessageKey()) as! String) { (index, title) in
                    }
                }
                else if let resAry = result as? NSArray {
                    UtilityClass.setCustomAlert(title: "Done", message: (resAry.object(at: 0) as! NSDictionary).object(forKey: GetResponseMessageKey()) as! String) { (index, title) in
                    }
                }
